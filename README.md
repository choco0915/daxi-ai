...
## V18：整理 data.md、交通直接回答、行程模板修正

V18 將使用者提供的舊版 Markdown 中重複的景點敘述、交通、停車、公車、雨天、無障礙與半日／一日／兩天一夜資訊重新整理進 `data.md`，並把實用資訊拆成獨立 RAG 章節。

主要行為：

- 景點第一次詢問只顯示摘要、關鍵資訊；說「再詳細一點」才展開整個章節。
- `交通`、`停車`、`公車`、`雨天`、`無障礙` 第一輪直接回完整實用資訊，不再要求使用者追問「再詳細一點」。
- `幫我安排大溪老街一日遊` 會直接使用 `大溪一日遊` 模板，列出老街、廟宇、午餐、木博館、人物文化與大溪橋等多站內容，不再因模糊官方實體匹配只得到單一「大溪老街油飯」。
- 「把剛剛推薦的第二個和第三個排成路線」等多輪自訂行程，仍保留原本的對話記憶與座標排序功能。
- 詳細介紹本身不再被視為「明確要求上網」；若 `data.md` 已有完整資料，會先使用本地知識庫，只有詢問最新營業、今日班次、當日活動等會變資訊時才優先查官方新資料。
