...
## V17：把核心店家與景點真正寫進 data.md

V17 不再把大部分常見店家問題寄望於即時外部搜尋，而是把大溪老街遊客常問的美食、伴手禮、木藝店家與主要景點整理成 `data.md` 的獨立 `##` 章節。每個主要店家盡量包含：
...
本版新增／強化的知識包含豆干老店、現滷豆干、月光餅、花生糖、豆花、碗粿、三角湯圓、豬腳、油飯、糕餅、西點、蜂蜜、辣醬、木器與工藝店家，以及大溪老街、普濟堂、福仁宮、中正公園、大溪橋、武德殿、公會堂、木博館、六廿四故事館、藝師館等主要景點。

### 「再詳細一點」現在真的會變詳細

第一次詢問單一店家／景點時，系統先回覆較短摘要與地址／電話／時間；如果接著說：
...
系統會沿用 `active_topics`，直接取同一個 `data.md` 章節的完整內容，顯示背景、特色、導覽重點與實用資訊，不再重複第一輪短回答。對已有完整 `data.md` 章節的主題，單純要求「詳細一點」也不會強制上網，因此 OpenAI quota 不足時仍能正常使用。

### RAG 結構調整

- `###` 子標題現在保留在 RAG chunk 中，前端會以真正的小標題顯示。
- 「豬腳、蜂蜜、月光餅、豆花、碗粿、拿破崙派」等短詞可直接命中對應店家獨立章節。
- 新增 `大溪必吃美食總覽`，處理「大溪有哪些必吃美食？」這類廣泛問題。
- 提高低相關命中的最低門檻，避免「火星基地」因為碰巧有「基地」兩字而錯配到工藝基地。
